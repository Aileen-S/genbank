...
nucgenes = ['18S', 'EF1A', 'H3', 'Wg']
mitogenes = ['12S', '16S', 'ATP6', 'ATP8', 'COX1', 'COX2', 'COX3', 'CYTB', 'ND1', 'ND2', 'ND3', 'ND4', 'ND4L', 'ND5', 'ND6']
sequencedict = {}
file = open("metadata.csv", "a")
writer = csv.writer(file)
for tax, rec in species.items():
    n = 0
    m = 0
    for gbid, output in rec.items():
        for gene in nucgenes:
            if gene in output["genes"]:
                n += 1
        for gene in mitogenes:
            if gene in output["genes"]:
                m += 1
    if n == 0 or m == 0:
        continue

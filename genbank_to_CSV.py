...
with open("COX1_longest.CSV", "w") as file:                                         # Open output file
    ...
    csvfile.writerow(["Accessions", "Family", "Subfamily", "Tribe", "Genus",])      # Specify column names
    for record in SeqIO.parse("COX1_longest.gb", "gb"):                             # Open GenBank file
        ...
        Row = [Acc]                                     # New list with accession in first column
        n = 0                                           # Start counter
        for t in Tax:
            n += 1                                      # +1 to counter for every loop
            if n >= 13:                                 # Skip taxon levels we don't need
